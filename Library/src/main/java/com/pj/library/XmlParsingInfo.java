...
public class XmlParsingInfo {
	...
	public List<BookDto> getData(int page, int perPage) {
		...
		try {
			String url = "https://api.odcloud.kr/api/15112642/v1/uddi:253ba864-742b-4ee3-8a8a-bb8662eacd02?page="+page+"&perPage="+perPage+"&serviceKey=wOrpV%2BvcpiMB64exxi3DKh0bX0cyBJFRLolt8baWBWEl%2FdbADWrfKNEMi%2B1UoP8PNOTEf6QlBVy8bEHkC%2BQKLQ%3D%3D";
			...
			for (int temp = 0; temp < nList.getLength(); temp++) {
				...
				if (nNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					Element eElement = (Element) nNode;
						...
						dto.set대분류(getTagValue("대분류", eElement)); // title이라는 태그 이름을 가진 eElement에서 값을 가져와서 dto객체의
						dto.set도서관구분코드(Integer.parseInt(getTagValue("도서관구분코드", eElement)));
						dto.set도서관명(getTagValue("도서관명", eElement));
						dto.set도서관홈페이지url(getTagValue("도서관홈페이지url", eElement));
						dto.set번호(Integer.parseInt(getTagValue("번호", eElement)));
						dto.set제목(getTagValue("제목", eElement));
						dto.set중분류(getTagValue("중분류", eElement));
						dto.set출판사(getTagValue("출판사", eElement));
						dto.set형식(getTagValue("형식", eElement));
						list.add(dto);
					}
				} // for end
